...
for workflow in data["workflow"]:
    ...
    scip.addCons(a[i][j] >= x[i] - x[j])
    scip.addCons(a[i][j] >= x[j] - x[i])
    scip.addCons(b[i][j] >= y[i] - y[j])
    scip.addCons(b[i][j] >= y[j] - y[i])
...
for j in range(N):
    for i in range(N):
        scip.addCons(
            x[i]
            + 0.5 * data["task_nodes"][i]["w"] * (1 - r[i])
            + 0.5 * data["task_nodes"][i]["h"] * r[i]
            <= x[j]
            - 0.5 * data["task_nodes"][j]["w"] * (1 - r[j])
            - 0.5 * data["task_nodes"][j]["h"] * r[j]
            + 1000000 * (1 - l[i][j])
        )
        scip.addCons(
            y[i]
            + 0.5 * data["task_nodes"][i]["h"] * (1 - r[i])
            + 0.5 * data["task_nodes"][i]["w"] * r[i]
            <= y[j]
            - 0.5 * data["task_nodes"][j]["h"] * (1 - r[j])
            - 0.5 * data["task_nodes"][j]["w"] * r[j]
            + 1000000 * (1 - m[i][j])
        )
...
for t in range(T):
    for i in range(N):
        ...
        scip.addCons(
            ...
            - 0.5 * data["task_nodes"][i]["w"] * (1 - r[i])
            - 0.5 * data["task_nodes"][i]["h"] * r[i]
            >= data["buildings"][t]["x"] - 1000000 * (1 - z[i][t])
        )
        ...
        scip.addCons(
            ...
            - 0.5 * data["task_nodes"][i]["h"] * (1 - r[i])
            - 0.5 * data["task_nodes"][i]["w"] * r[i]
            >= data["buildings"][t]["y"] - 1000000 * (1 - z[i][t])
        )
...
for i in range(N):
    ...
    if scip.getVal(r[i]) == 1:
        ax.add_patch(
            Rectangle(
                (
                    scip.getVal(x[i]) - 0.5 * data["task_nodes"][i]["h"],
                    scip.getVal(y[i]) - 0.5 * data["task_nodes"][i]["w"],
                ),
                data["task_nodes"][i]["h"],
                data["task_nodes"][i]["w"],
                edgecolor="pink",
        ...
        ax.add_patch(
            Rectangle(
                (
                    scip.getVal(x[i]) - 0.5 * data["task_nodes"][i]["w"],
                    scip.getVal(y[i]) - 0.5 * data["task_nodes"][i]["h"],
                ),
                data["task_nodes"][i]["w"],
                data["task_nodes"][i]["h"],
                edgecolor="pink",
