...
LPModel::LPModel(std::shared_ptr<instance::Instance> inst,
...
void LPModel::AddConstriants() {
  ...
  for (int i = 0; i < N; ++i) {
    for (int t = 0; t < T; ++t) {
      GRBLinExpr expr = (*z)[i][t];
      model->addConstr(expr <= (*p)[t], fmt::format("facility_meet_{}", i));
    }
  }
  ...
  for (int t = 0; t < T; ++t) {
    for (int j = 0; j < N; ++j) {
      for (int i = 0; i < j; ++i) {
        ...
        model->addConstr(expr >= 1, fmt::format("tasknode_relations_{}_{}_{}", i, j, t));
      }
    }
  }
  ...
  for (int j = 0; j < N; ++j) {
    for (int i = 0; i < N; ++i) {
      ...
      model->addConstr(x[i] + w_i * (1 - (*r)[i]) + h_i * (*r)[i]
                         <= x[j] + 1000000 * (1 - (*l)[i][j]),
                       fmt::format("task-task-right_{}_left_{}", i, j));

      model->addConstr(y[i] + h_i * (1 - (*r)[i]) + w_i * (*r)[i]
                         <= y[j] + 1000000 * (1 - (*m)[i][j]),
